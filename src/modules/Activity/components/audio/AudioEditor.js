...
const AudioEditor = ({ courseId, activityId, handleError }) => {
  ...
  const contentOverview = activityData
    ? JSON.parse(activityData.contentOverview)
    : [];
    ...
    <div className={styles.audioEditor}>
      <div className={styles.tabs}>
        ...
        <button
          ...
          className={activeTab === "script" ? styles.active : ""}
        >
          {t("script")}
        </button>
        <button
          ...
          {activityData ? (
            <>
              <div className={styles.activityDetails}>
                <p>
                  <strong>{t("duration")}</strong> {activityData.duration}{" "}
                  {t("minutes")}
                </p>
                <p>
                  <strong>{t("format")}</strong> {activityData.contentType}
                </p>
              </div>
              ...
              {activityAudio ? (
                <Button2
                  className={styles.generateActivityButton}
          ...
          {activityAudio && activityAudio.filePath ? (
            <>
              <audio controls className={styles.audioPlayer}>
                <source
                  src={`${process.env.NEXT_PUBLIC_API_URL}/v1/download/audio/file/${activityData.id}?token=${fileToken}`}
                  type="audio/mp3"
                />
                Your browser does not support the audio element.
              </audio>
              <a
                href={`${process.env.NEXT_PUBLIC_API_URL}/v1/download/audio/file/${activityData.id}?token=${fileToken}`}
